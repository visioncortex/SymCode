...
pub trait GlyphReader {
    ...
    fn find_most_similar_glyph(glyph_library: &Self::Library, image: BinaryImage, params: Self::Params) -> Self::Label;
    ...
    fn read_glyphs_from_rectified_image(image: visioncortex::BinaryImage, glyph_library: &Self::Library, symcode_config: &crate::scanning::SymcodeConfig) -> Vec<Option<Self::Label>> {
        ...
        symcode_config.glyph_anchors.iter()
            .map(|anchor| {
                ...
                Some(Self::find_most_similar_glyph(crop, symcode_config.stat_tolerance, symcode_config.max_encoding_difference))
            })
            .collect()
    }
}